...
export class SimulatorEditorPageComponent implements OnInit {
  ...
  set selectedLayer(layer) {
    if (layer) {
      ...
      this.selectedLayerInSimulator = this.layersInSimulator.find(
        layerIS => layerIS.layer_id === layer.layer_id
      );
    } else {
  ...
  reorderLayers($event) {
    ...
    const layers = this.layers.filter(
      (layer, index) => index !== previousIndex
    );
  ...
  openAddLayerModal() {
    const modalRef = this.modalService.open(LayerCreateComponent, {
  ...
  resetLayers() {
    this.layersInSimulator = this.layers.map(layer => {
      ...
      newLayer.currentItem = layer.items.find(
        item =>
          layer.default_item &&
          item.item_id.toString() === layer.default_item.toString()
      );
      return newLayer;
    });
  }
